  # ESERCIZIO FATTO DURANTE IL CORSO INGEGNERIA DEL SOFTWARE A.A. 2022/23

## Forth Interpreter

### Processo

Implementa secondo la *metodologia TDD* le specifiche riportate di seguito; 
in maggior dettaglio, ripetete i passi seguenti fino ad aver implementato tutte le funzionalità richieste:

* implementare un test per le funzionalità richieste **procedendo nell'ordine** in cui sono specificate,
* aggiungere la minima implementazione necessaria a realizzare la funzionalità, in modo che **il test esegua con successo**;
...
Al termine del laboratorio effettuare un ultimo *push* e **verificare che ci sia la completa traccia di *commit* effettuati. Si
suggerisce di eseguire i test non soltanto con Idea, ma anche eseguendo il comando `gradle test` da riga di comando.
...
In presenza di *code smell*, effettuare alcuni passi di *refactoring*,
per ottenere codice più *leggibile* e *manutenibile*.

Di seguito si accenna ad alcune possibili azioni di refactoring.
Accanto al tipo di refactoring è stata elencata una delle [Refactor Actions](https://www.baeldung.com/intellij-refactoring) di Idea (se presente) che puoi usare.
